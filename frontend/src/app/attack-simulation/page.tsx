...
export default function AttackSimulationPage() {
  ...
  const reload = () => {
    fetchDashboardMetrics()
      .then(setData)
      .catch((e) => setError(e.message || "Failed to load simulation data"));
  };
  ...
  return (
    <div className="min-h-screen bg-[#090d16] text-slate-100 pb-12">
      <main className="max-w-6xl mx-auto px-6 pt-8 space-y-6">
        <h1 className="text-xl font-bold text-white">Attack Simulation & Defense Tester</h1>
        <AttackControlPanel onRoundComplete={reload} />
